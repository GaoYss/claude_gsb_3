...
| 药剂档案 | `/pesticides` | 药剂建档（登记证号、毒性、剂型、防治对象、登记安全间隔期）、库存余额与低库存预警、入库补货与领用出库登记、库存自动扣减/回滚 |
| 施药记录 | `/pesticide-applications` | 施药登记（施药区域、防治对象、稀释浓度、用药量、施药人员）、按安全间隔期自动推算最早可进入时间、间隔期内同区域再次施药软警告二次确认、间隔期状态筛选与提醒 |
...
│   │   │   ├── pesticide_service.py   # 药剂档案 / 出入库库存 / 施药间隔期
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、15 条任务、22 条养护记录、8 条更换记录、7 种药剂、约 29 条出入库与 25 条施药记录，含库存预警与间隔期内重复施药场景）。停止与清理：
...
| GET/POST | `/pesticides` | 药剂档案列表（`keyword`/`pesticide_type`/`toxicity`/`low_stock`，返回库存汇总） / 新建药剂（编号自动、间隔期必填） |
| GET | `/pesticides/options` | 药剂下拉选项（带当前库存与安全间隔期） |
| GET/PUT/DELETE | `/pesticides/{id}` | 药剂详情（含近期出入库/施药） / 更新（库存不可直接改） / 删除（有流水或施药需 `force`） |
| GET/POST | `/pesticide-stock-movements` | 出入库流水列表（`pesticide_id`/`green_space_id`/`movement_type`/日期区间） / 入库或领用（领用超库存返回 409） |
| DELETE | `/pesticide-stock-movements/{id}` | 删除出入库记录并自动回滚库存（不支持修改） |
| GET/POST | `/pesticide-applications` | 施药记录列表（`green_space_id`/`pesticide_id`/`interval_status`/日期区间，返回汇总） / 登记施药 |
| GET | `/pesticide-applications/interval-check` | 施药前实时预检：返回该区域所选时刻仍在间隔期内的记录 |
| GET/PUT/DELETE | `/pesticide-applications/{id}` | 详情 / 更新（重算最早可进入时间） / 删除 |
| POST | `/pesticide-applications?confirm=true` | 间隔期内同区域再次施药，二次确认后登记 |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 任务提醒 + 药剂库存/间隔期提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口（`/reminders` 含低库存药剂与仍在间隔期内的施药区域） |
...
8. **药剂库存可追溯**：药剂建档时录入期初库存与预警阈值，建档后库存不可直接修改，只能通过入库补货（`in`）增加、领用出库（`out`）扣减；领用数量超过当前库存返回 409 并阻止出库。出入库流水不支持修改，删除即自动反向回滚库存（入库单删除扣回、领用单删除补回），杜绝库存二次增减。
9. **施药安全间隔期**：施药记录保存时从药剂档案快照药剂名称、计量单位与安全间隔期（天）；最早可进入时间 = 施药日期 + 可选时刻 + 间隔天数（未填时刻按当日 00:00）。药剂档案之后被删除或调整不影响历史履历。
10. **同区域重复施药提示**：同一绿地在所选施药时刻仍存在「最早可进入时间」未到的施药记录时，保存先返回 409 软警告（列出冲突的药剂、上次施药日期与最早可进入时间），前端弹窗提示做好安全防护，用户二次确认（`confirm=true`）后方可登记；不同绿地互不影响。前端在选择区域/日期/时刻时还会调用 `/pesticide-applications/interval-check` 实时预检。
11. **施药履历独立留存**：删除药剂档案时其出入库流水级联清除，但施药记录保留（外键 `SET NULL`，名称与间隔期走快照）；删除绿地时其施药记录随绿地级联清除，并计入绿地删除保护的关联数量。
...
| `pesticide` | 药剂档案 | `code`(唯一,PC-年份-序号)、`name`、`pesticide_type`、`toxicity`、`form`、`registration_no`、`unit`、`stock_quantity`、`stock_low_threshold`、`safety_interval_days` |
| `pesticide_stock_movement` | 药剂出入库流水 | `movement_no`(唯一,SM-日期-序号)、`pesticide_id`、`movement_type`(in/out)、`quantity`、`movement_date`、`receiver`、`green_space_id`(可空,SET NULL)、`purpose` |
| `pesticide_application` | 施药记录 | `application_no`(唯一,PA-日期-序号)、`green_space_id`、`pesticide_id`(可空,SET NULL)、`pesticide_name`(快照)、`location`、`application_date`、`application_time`、`target_pest`、`dilution_ratio`、`dosage`、`safety_interval_days`(快照)、`earliest_reentry_at`、`operator` |

绿地删除时任务/记录/更换/施药记录级联清理；任务与养护记录之间、养护记录与更换记录之间、药剂档案与施药记录之间、绿地与出入库流水之间为可空外键（`SET NULL`），保证养护与施药履历可独立留存。药剂删除时其出入库流水随档案级联清除。

## 八、测试

```bash
cd backend
python -m pytest              # 73 个用例：接口、校验、跨模块规则、药剂库存与间隔期、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、药剂入库/领用的库存扣减与不足拦截、出入库删除回滚、最早可进入时间推算、间隔期内同区域重复施药的软警告与二次确认、删除档案后施药履历快照留存、统计聚合口径一致性、演示数据自洽性。
